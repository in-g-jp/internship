# 0303 Paiza 問題への取り組み方

## 1. 取り組む問題について

スキルチェック→プログラミング言語から問題を選択
...
先ず、画面左のタブからソース管理の画面へ移行します。

下の添付画像のように変更タブ下の窓に、"メッセージ(Enterで'main'にコミット)"と薄文字で記載されていることを確認してください。

（'main'ではなく、他の文言が入っていれば、変更タブの右端にある三点リーダーをクリック。「チェックアウト先...」を選択し、検索窓で'main'と入力後、先ほどのメッセージがmainと変更されていることを確認してください。）

次に、変更タブの右端にある三点リーダーをクリックして「チェックアウト先...」を選択してください。
...
### 2-8. Claude のレビュー内容を確認・修正する

PR を作成すると、Claude という AI が自動でコードレビューを行います。

1. Claude からのレビューコメントを読み、作業ブランチ上でコードを修正する
2. 修正したらそのまま作業ブランチをコミット・プッシュする
3. 修正が完了したレビューコメントは「Resolve conversation」ボタンを押して閉じる
4. Claude からのすべてのレビューの修正が終わったら、「Reviewers」に **神谷** を設定して再レビューを依頼する
